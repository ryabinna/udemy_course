...
const actions = {
    [actionTypes.updateArticle](context, {slug, articleInput}){
        return new Promise(resolve => {
            ...
            state.article.title = articleInput.title;
            state.article.description = articleInput.description;
            state.article.body = articleInput.body;
            state.article.tagList = articleInput.tagList;
            articleApi.updateArticle(slug, {article:state.article})
            .then(article => {context.commit(mutationTypes.updateArticleSuccess, article)
            resolve(article)})
    ...
    [actionTypes.getArticle](context, slug){
        return new Promise(resolve => {
            ...
            .then(article => {context.commit(mutationTypes.getArticleSuccess, article)
            resolve(article)})
